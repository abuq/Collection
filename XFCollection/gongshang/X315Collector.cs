...
namespace XFCollection.gongshang
{
    ...
    /// X315Collector
    /// </summary>
    public class X315Collector :WebRequestCollector<IResut, X315Parameter>
    {
        ...
        internal void Test()
        {
            ...
            TestHelp<X315Collector>(parameter);
        }
        ...
        private string GetHtmlByGet(string url)
        {
            ...
            {
                //如果是500错误 404错误
                if (e.Message.Contains("500")|| e.Message.Contains("404"))
                {
                    throw new Exception("被屏蔽了。");
                }
            }

            if (Regex.Match(html, @"<div class=""f_h25 f_g1 f_w_b"">[\s\S]*?</div>").Value.Contains("太厉害了! 您的查询速度已经超过"))
            {
                throw new Exception("被屏蔽了。");
            }
